  ...
  },
  {
    ...
    icon: (active: boolean) => (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth={active ? 0 : 1.5} className="w-6 h-6">
        ...
        {active && <circle cx="7" cy="17.25" r="2" fill="currentColor"/>}
      </svg>
    ),
  },
  {
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-sand-200 z-50 safe-area-bottom">
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center py-2 px-4 min-w-[64px] transition-colors ${
                isActive
                  ? "text-slate-blue"
                  : "text-sand-400 hover:text-slate-blue/70"
              }`}
            >
              {tab.icon(isActive)}
